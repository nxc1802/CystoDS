...
# %%
# CELL 2 - All finalist cross-validation configuration.
import os
...
def _make_base_config() -> dict:
    config = {
        ...
        "protocol": (
            "holdout" if RUN_PROFILE == "smoke" else "cross_validation"
        ),
        "train_fraction": 0.70,
        "val_fraction": 0.15,
        "test_fraction": 0.15,
        ...
        # Defaults target a single 96 GB CUDA accelerator. Every fold emits
        # its complete metrics/predictions; the suite emits the final report.
        "batch_size": int(os.environ.get("CYSTODS_BATCH_SIZE", "256")),
        "num_workers": int(os.environ.get("CYSTODS_NUM_WORKERS", "16")),
        "prefetch_factor": int(
...
CONFIG = {
    ...
    "fold_ids": (0,) if RUN_PROFILE == "smoke" else None,
...
try:
    ...
    _THIS_SOURCE = (
        Path.cwd() / "stage_90_run_cross_validation_and_report.py"
    ).resolve()
...
if __name__ == "__main__":
    ...
    print(f"Completed cross-validation stage: {COMPLETED_RUN_DIRECTORY}")
