...
# %%
# CELL 2 - All long-tail screening configuration.
import os
...
def _make_base_config() -> dict:
    config = {
        ...
        "batch_size": int(os.environ.get("CYSTODS_BATCH_SIZE", "512")),
        ...
        "monitor_metric": "fine_macro_f1_all_classes",
        ...
        # This stage exists to expose rare-class collapse. Violations are real
        # results to report across every loss, not reasons to abort the suite.
...
RESEARCH_TRIALS = tuple(
    {
        "experiment_id": experiment_id,
        "task_mode": "fine",
        "overrides": overrides,
    }
    for experiment_id, overrides in (
        ("fine_cross_entropy", {"fine_loss": "cross_entropy"}),
        (
            "fine_weighted_ce",
            {"fine_loss": "weighted_ce", "class_balance_beta": 0.9999},
        ),
        ("fine_focal", {"fine_loss": "focal", "focal_gamma": 2.0}),
        (
            "fine_balanced_softmax",
            {"fine_loss": "balanced_softmax"},
        ),
        (
            "fine_balanced_softmax_smoothed",
            {
...
SMOKE_TRIALS = (
    {
        "experiment_id": "smoke_fine_cross_entropy",
        "task_mode": "fine",
        "overrides": {
            "model_name": "swin_tiny_patch4_window7_224.ms_in1k",
            "pretrained": False,
    ...
    {
        "experiment_id": "smoke_fine_smoothed",
        "task_mode": "fine",
        "overrides": {
            "model_name": "swin_tiny_patch4_window7_224.ms_in1k",
            "pretrained": False,
...
CONFIG = {
    ...
    "trials": SMOKE_TRIALS if RUN_PROFILE == "smoke" else RESEARCH_TRIALS,
...
try:
    ...
    _THIS_SOURCE = (
        Path.cwd() / "stage_20_run_long_tail_screen.py"
    ).resolve()
...
if __name__ == "__main__":
    ...
    print(f"Completed long-tail screen: {COMPLETED_RUN_DIRECTORY}")
