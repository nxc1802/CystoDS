...
# %%
# CELL 2 - All baseline-suite configuration.
import os
...
def _make_base_config() -> dict:
    config = {
        ...
        # Defaults target a single 96 GB CUDA accelerator. Override after a
        # measured OOM/throughput check rather than silently shrinking work.
        "num_workers": int(os.environ.get("CYSTODS_NUM_WORKERS", "4")),
        "prefetch_factor": int(os.environ.get("CYSTODS_PREFETCH_FACTOR", "2")) if int(os.environ.get("CYSTODS_NUM_WORKERS", "4")) > 0 else None,
        "eval_batch_size": int(
        ...
        "eval_prefetch_factor": int(os.environ.get("CYSTODS_EVAL_PREFETCH_FACTOR", "2")) if int(os.environ.get("CYSTODS_EVAL_NUM_WORKERS", "4")) > 0 else None,
        "persistent_workers": int(os.environ.get("CYSTODS_NUM_WORKERS", "4")) > 0,
        "pin_memory": True,
        "model_name": os.environ.get(
            "CYSTODS_MODEL_NAME", "swin_tiny_patch4_window7_224.ms_in1k"
        ),
        "pretrained": True,
        "task_mode": "binary",
        "dropout": 0.20,
        ...
        "weight_decay": 0.05,
        "optimizer": "adamw",
        "use_fused_optimizer": _strict_env_bool(
            "CYSTODS_USE_FUSED_OPTIMIZER", "true"
        ),
        "warmup_epochs": 2.0,
        "scheduler_epochs": 25,
...
try:
    ...
    _THIS_SOURCE = (Path.cwd() / "stage_10_run_baselines.py").resolve()
...
if __name__ == "__main__":
    ...
    print(f"Completed baseline stage: {COMPLETED_RUN_DIRECTORY}")
