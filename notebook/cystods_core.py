...
#
# This is intentionally the first cell so the same Python file can be imported
# into Kaggle/Jupyter as a percent-format pre-notebook. Packages are installed
# only when the active environment does not already satisfy the version range.
...
import cystods_hf as checkpoint_hub
...
def make_run_directory(config: Mapping[str, Any]) -> Path:
    ...
    base = root / f"{config['experiment_name']}_{config['run_profile']}_{stamp}"
    run_dir = base
    suffix = 1
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        try:
            with Image.open(path) as source:
                ...
                view_two = self.transform(image) if self.second_view else None
        except Exception as exc:
...
def build_dataloaders(
    ...
    second_view = config["supervised_contrastive_loss_weight"] > 0
    datasets = {
        "train": CystoDataset(
            split_frames["train"], train_transform, second_view=second_view
        ),
        "val": CystoDataset(
            split_frames["val"], eval_transform, second_view=False
        ),
    }
...
def train_model(
    ...
    for epoch in range(start_epoch, int(config["epochs"])):
        ...
        for batch_index, batch in enumerate(loaders["train"]):
            ...
            binary_targets = binary_targets_one.repeat(repeat)
            coarse_targets = coarse_targets_one.repeat(repeat)
            fine_targets = fine_targets_one.repeat(repeat)
            ...
            with torch.autocast(
                ...
                if supcon_weight > 0:
                    ...
                    if (
                        ...
                        supcon_valid = fine_targets >= 0
                        supcon_labels = fine_targets[supcon_valid]
...
def make_deterministic_eval_loader(
    ...
    dataset = CystoDataset(frame, eval_transform, second_view=False)
    num_workers = (
        0 if device.type == "mps" else int(config["eval_num_workers"])
    )
    kwargs: dict[str, Any] = {
...
def main(
    ...
    if required_source_files is None:
        ...
        required_source_files = (
            core_source,
            core_source.with_name("cystods_science.py"),
            core_source.with_name("cystods_hf.py"),
        )
